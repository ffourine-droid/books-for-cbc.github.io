      ...
      <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-2xl z-50 flex flex-col border-l border-indigo-100 animate-in slide-in-from-right duration-300">
        <div className="p-4 border-b bg-indigo-600 text-white flex justify-between items-center">
          <div className="flex items-center gap-2">
            <Icons.Tutor />
            <span className="font-bold text-sm sm:text-base">MathMaster AI</span>
          </div>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] rounded-2xl p-3 text-sm ${
                ...
                <div className="prose prose-sm max-w-none whitespace-pre-wrap leading-relaxed text-[13px] sm:text-sm">
                  {msg.content}
                </div>
              </div>
            </div>
        ...
        <div className="p-4 border-t bg-white safe-area-bottom">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2.5 bg-slate-100 border-none rounded-full text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none"
            />
            <button 
              onClick={handleSend}
              disabled={isLoading}
              className="p-2.5 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 disabled:opacity-50 transition shadow-md active:scale-90"
            >
